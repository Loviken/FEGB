...
class Basic:
	...
	def set_title(self, frame_id, title):
		frame_displace = self.get_displacement(frame_id)
		canvas_side	   = 1.*self.canvas_prop*self.frameside
		...
		self.display_message(title, text_center)
	...
	def draw_cross(self, frame_id, x, color, size = 4):
		
		frame_displace  = self.get_displacement(frame_id)
		canvas_displace	= np.array([(1. - self.canvas_prop)*self.frameside/2, self.circ_thick])

		pos_relative  = x*self.frameside*self.canvas_prop
		pos  		  = frame_displace + canvas_displace + pos_relative
		pos  		  = np.int32(pos + 0.5) # round

		o1 = size*np.array([1,1])
		o2 = size*np.array([-1,1])
		
		pg.draw.line(self.screen, color, pos-o1, pos+o1, 4)
		pg.draw.line(self.screen, color, pos-o2, pos+o2, 4)
	...
	def check_events(self):
		...
		for event in pg.event.get():
			...
			if event.type == QUIT:
				...
				[frame_id, frame_pos] = self.get_frame_and_pos(window_target)
				
				return self.get_x_from_frame(frame_pos)
