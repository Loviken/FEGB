...
class Basic:
	def __init__(self, frameside, frame_grid, the_caption, id_gui = -1, buttons = ()):
		pg.init()
		...
		pg.display.set_caption(the_caption)
		pg.display.update()
	...
	def get_g_from_frame(self, frame_pos):
		frame_pos       = np.array(frame_pos)
		canvas_displace = np.array([(1. - self.canvas_prop)*self.frameside/2, self.circ_thick])
		
		g_coord = 1.*(frame_pos - canvas_displace)/(self.canvas_prop*self.frameside)
	...
	def draw_matrix(self, matrix, frame_id, v_min = 1337, v_max = 1337, matrix_text = ''):
		...
		# Draw background
		circumfence = np.append(frame_displace + canvas_displace - circ_thick , \
								np.array([canvas_side, canvas_side]) + circ_thick*2)
		pg.draw.rect(self.screen, (0, 0, 0), circumfence)
		#
	...
	def draw_g_states(self, frame_id, g_now = [], g_goal = [], color_goal = RED):

		frame_displace  = self.get_displacement(frame_id)
		canvas_displace	= np.array([(1. - self.canvas_prop)*self.frameside/2, self.circ_thick])

		if len(g_now) > 0:
			pos_now_relative  = g_now*self.frameside*self.canvas_prop
			pos_now  = frame_displace + canvas_displace + pos_now_relative
			pos_now  = np.int32(pos_now + 0.5)
			
			pg.draw.circle(self.screen, BLUE, pos_now, 8)
		
		if len(g_goal) > 0:
			pos_goal_relative = g_goal*self.frameside*self.canvas_prop
			pos_goal = frame_displace + canvas_displace + pos_goal_relative
			pos_goal = np.int32(pos_goal + 0.5)
			
			pg.draw.circle(self.screen, color_goal, pos_goal, 4)
...
'''
bc = Basic(200, (3, 4), "Whatever")

for i in range(13):
	print(bc.get_start_pos())
